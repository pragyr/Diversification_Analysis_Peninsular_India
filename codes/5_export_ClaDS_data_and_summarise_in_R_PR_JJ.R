...
setwd(paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/../data/all_trees_or_super_trees"))
...
clads.rates.all <- matrix(ncol=4)
colnames(clads.rates.matrix) <- c("Lineage","Mean_Speciation_Rate","Mean_Extinction_Rate","Mean_Net-diversification_Rate")
colnames(clads.rates.all) <- c("Lineage","Speciation_Rate","Extinction_Rate","Net-diversification_Rate")
...
for(j in 1:length(filenames)){
  ...
  for(k in 1:l.nodes){
    ...
    descendant.only.tips <- unlist(Descendants(tree,nodes[k],"tips"))
    
    sp.rate.tips <- CladsOutput$lambdatip_map[descendant.only.tips]
    ex.rate.tips <- CladsOutput$eps_map*sp.rate.tips
    nd.rate.tips <- sp.rate.tips - ex.rate.tips
    
    clads.rates.all <- rbind(clads.rates.all,cbind(rep(names(nodes)[k],length(descendant.only.tips)),
                                                   sp.rate.tips,ex.rate.tips,nd.rate.tips))
...
clads.rates.all <- clads.rates.all[-1,]
clads.rates.all <- clads.rates.all[order(clads.rates.all[,"Lineage"]),]
View(clads.rates.matrix)
write.csv(clads.rates.matrix, "../clads_mean_rates.csv",row.names = F)
write.csv(clads.rates.all,"../clads_tip_rates.csv",row.names = F)
