...
for(i in 1:length(filenames)){
  ...
  while(!convergence){
    ...
    if(length(which(ess.vals <= 200)) == 0){
      ...
      write.csv(sp.rates.df,file=paste0("../../RTT_data/",lineage.names[i],"_sp_rates.csv"))
      write.csv(ex.rates.df,file=paste0("../../RTT_data/",lineage.names[i],"_ex_rates.csv"))
      write.csv(sp.shift.df,file=paste0("../../RTT_data/",lineage.names[i],"_sp_shift_times.csv"))
      write.csv(ex.shift.df,file=paste0("../../RTT_data/",lineage.names[i],"_ex_shift_times.csv"))
      write.csv(bayes.factors,file=paste0("../../RTT_data/",lineage.names[i],"_bayes_factors.csv"))
    }
  }
  
  ### PLOTS TO CHECK FOR CONVERGENCE
  CairoPDF(paste0(lineage.names[i],"_comet_no_mass_extinctions_convergence.pdf"), 6, 6, bg="transparent")
  ...
  ### PLOTS FOR RATES
  CairoPDF(paste0(lineage.names[i],"_speciation_rates.pdf"), 6, 6, bg="transparent")
  ...
  CairoPDF(paste0("../../CRABS_results/",lineage.names[i],"_CRABS_output_summary.pdf"), 6, 6, bg="transparent")
  plot(p)
  # dev.off()
  # tiff(paste0("../",lineage.names[i],"_CRABS_output_summary2.tif"), 6, 6, bg="transparent")
  plot(my_model)
  # dev.off()
  # tiff(paste0("../",lineage.names[i],"_CRABS_output_summary3.tif"), 6, 6, bg="transparent")
  ps_lambdas
  ps_mus
  ps_deltas
  dev.off()
...
write.csv(mean.rates,"../Mean_rates.csv",row.names = F)

# extract bayes factors through time bins
bayes_factor_files <- list.files("../RTT_data/",pattern="bayes")
...
for(i in 1:length(bayes_factor_files)){
  #i=23
  bf <- read.csv(paste0("../RTT_data/",bayes_factor_files[i]))[,-1]
  sp.bf <- bf[1,]
  ex.bf <- bf[2,]
