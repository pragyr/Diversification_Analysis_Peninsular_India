...
library(FactoMineR)
library(factoextra)
library(ecole) # to install, execute this command: remotes::install_github("phytomosaic/ecole")
               # if you don't have the remotes package then install it using: install.packages("remotes")
library(ggstatsplot)
library(ggsci)
library(qpdf)
library(ggtree)
...
dir.create("../graphs")
dir.create("../graphs/supplementary")
...
# function to perform PGLS and make a regression plot
mk_pgls_plot <- function(formula,phy,df.pgls,xlab,ylab,make.plot=TRUE,xlim.min,ylim.min){
  ...
  get_breaks_custom <- function(vals,lab){
    if(!(-1 %in% unlist(gregexpr("log",lab)))){
      max <- exp(max(vals))
      num_integers.min <- num_integers(exp(min(vals)))
      num_integers.max <- num_integers(max)
      
      diff <- 10^num_integers.min-as.integer(max%%10^num_integers.min)
      
      log(c(1,seq((10^(num_integers.min+1))/2,as.integer(max)+diff,(10^(num_integers.min+1))/2)))
      
    }else{
      max <- max(vals)
      min <- min(vals)
      
      num_integers.min <- num_integers(abs(min))
      num_integers.max <- num_integers(abs(max))
      ...
      if(max<1){
        ...
        min <- ifelse(min>=0,0,sign["min"]*round(abs(min)*10^-num_integers.min,0)/10^-num_integers.min)
        max <- ifelse(max>=0,sign["max"]*round(abs(max)*10^-num_integers.min,0)/10^-num_integers.min,0)
        interval <- round((max-min)/5,-ifelse(min < 0,num_integers.min,num_integers((max-min)/5)))
        
        seq(min,max+interval,interval)
      }else{
        max <- as.integer(max)
        diff <- 10^num_integers.min-as.integer(max%%10^num_integers.min)
        max <- round(max+diff,abs(num_integers.min))
        min <- 0
        interval <- round((max-min)/5,-num_integers.min)
  ...
  plot <- ggplot(df.pgls,aes(x=Predictor,y=Response)) + 
    geom_point(aes(color=Biogeographic.origin,
                   ...
                   shape=Habitat),size=4,stroke=1.1) +
    scale_fill_manual(values = c(alpha("#00ff00",0.7),alpha("#000000",0.7),alpha("#0000ff",0.7)),
                      guide=guide_legend(override.aes = list(shape = 21)))+
    scale_shape_manual(values = c(21,22,24))+
    scale_color_manual(
      values = c("darkorange", "darkgrey"),
      guide = guide_legend(override.aes = list(shape = 21))
    )+
...
p1.tip_rate <- ggplot(data2,aes(x = Taxonomic.group, y = Net.diversification_Rate,fill = Taxonomic.group,color=Taxonomic.group))+
  ...
  scale_fill_manual(values = c("#00ff00","#000000","#0000ff"))+
  scale_color_manual(values = c("#00ff00","#000000","#0000ff"))+
  theme_classic()+
...
p4 <- mk_pgls_plot(Net.diversification.rate..CoMET. ~ Crown.age..Mya.,
...
CairoPDF("../graphs/2_Tip_and_clade_rate_vs_predictors.pdf",11.7,8.3,bg="transparent")
...
filenames.SC1 <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$Final.Results=="SC1")],"_nd_rates.csv")
filenames.SC2 <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$Final.Results=="SC2")],"_nd_rates.csv")
filenames.SC2_Shift <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$Final.Results=="SC2+Shift")],"_nd_rates.csv")
filenames.SC3_Shift <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$Final.Results=="SC3+Shift")],"_nd_rates.csv")
filenames.Shift <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$Final.Results=="Shift")],"_nd_rates.csv")
filenames.TDD <- paste0("./TESS_RTT_data/",data.taxa.modified$Lineage[which(data.taxa.modified$TDD=="TDD")],"_nd_rates.csv")
...
########################################## Figure 4: Multivariate-analysis using FAMD ##################################################
...
mk_FAMD_plot <- function(df,list.variables,palette){
  ...
  diss.matrix <- kmed::distmix(df,method = "gower",idnum = c(2,3,4),idbin = NULL,idcat = c(1,5:10)) # gower's distance because it's a mixed data
  ...
  CairoPDF("../graphs/supplementary/supplementary_FAMD.pdf",8.3,11.7,bg="transparent")
  ...
  if(is.null(list.variables)){
    ...
    if(!("PermANOVA_results" %in% list.files("../graphs/supplementary/"))){
      dir.create("../graphs/supplementary/PermANOVA_results")
    }
    write.csv(perm.var,paste0("../graphs/supplementary/PermANOVA_results/Permanova_",name,".csv"))
...
colnames(dt) <- c("Lineage","Taxonomic groups","Clade Age",
                  ...
                  "Biogeographic Origins","Habitat Types",
...
dev.off()
write.csv(dt,"../graphs/supplementary/FAMD_input_matrix.csv",row.names = F)
###########################################################################################################################
...
pd7 <- ggbetweenstats(
  ...
  ylim(c(0,0.5))+
  geom_text(aes(y=0.45,x=2,label=ifelse(pval7<0.05,as.character(as.expression((substitute(italic("p")~"="~p,list(p=pval7))))),"")
                #'paste(italic("p"),"=",pval)'
  ),
  size = 3,check_overlap = TRUE,color="black",parse=T)+
  theme_classic()+xlab("Biogeographic origins")+ylab("")+#ylab("Net-Diversification rate")+
...
pd11 <- ggbetweenstats(
  ...
  ylim(c(0,0.5))+
  geom_text(aes(y=0.45,x=2,label=ifelse(pval11<0.05,as.character(as.expression((substitute(italic("p")~"="~p,list(p=pval11))))),"")
                #'paste(italic("p"),"=",pval)'
  ),
  size = 3,check_overlap = TRUE,color="black",parse=T)+
  theme_classic()+xlab("Biogeographic origins")+ylab("")+#ylab("Net-Diversification rate")+
...
# install pdftk on your system to make this work. it should be available for all UNIX based OSs
